...
contract ParetoDollar is IParetoDollar, ERC20Upgradeable, ReentrancyGuardUpgradeable, EmergencyUtils {
  ...
  function mint(address collateralToken, uint256 amount) external nonReentrant returns (uint256 scaledAmount) {
    ...
    // mint ParetoDollars
    // added conditional to ensure info.token decimals is <= 18

    if (info.tokenDecimals > 18) {
      revert InvalidData();
    }
...
// ensures events and errors are defined 
...
event Minted(address indexed user, address indexed collateral, uint256 amount, uint256 scaledAmount);
event RedeemRequested(address indexed user, uint256 epoch, uint256 amount);
event Redeemed(address indexed user, uint256 epoch, uint256 amount);
event CollateralAdded(address indexed token, address indexed priceFeed, uint8 tokenDecimals, uint8 priceFeedDecimals, uint256 validityPeriod);
